...
public class QueryGenerator {
     ...
     */
    public static <T> void installMplus(QueryWrapper<T> queryWrapper, Object searchObj, Map<String, String[]> parameterMap) {
     ...
     */
    public static <T> void doSuperQuery(QueryWrapper<T> queryWrapper, Map<String, String[]> parameterMap, Map<String, String> fieldColumnMap) {
        if (parameterMap != null && parameterMap.containsKey(SUPER_QUERY_PARAMS)) {
            String        superQueryParams    = parameterMap.get(SUPER_QUERY_PARAMS)[0];
            String        superQueryMatchType = parameterMap.get(SUPER_QUERY_MATCH_TYPE) != null ? parameterMap.get(
                    SUPER_QUERY_MATCH_TYPE)[0] : MatchTypeEnum.AND.getValue();
            ...
            try {
                ...
                Consumer<QueryWrapper<T>> queryWrapperConsumer = andWrapper -> {
                    for (int i = 0; i < conditions.size(); i++) {
                        ...
                        if (oConvertUtils.isNotEmpty(rule.getField())
                            ...
                            if (rule.getField().contains(",")) {
                                ...
                                ((LtmaiQueryWrapper<?>) queryWrapper).inSubTable(matchType, split[0].trim(), split[1].trim(),
				...
				if (queryWrapper.getExpression().getNormal().isEmpty()) {
                    queryWrapper.func(queryWrapperConsumer);
				}
				else if (MatchTypeEnum.OR == matchType) {
